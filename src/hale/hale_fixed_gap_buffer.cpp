...
{
    ch8 *memory = (ch8*)platform.allocate_memory(buf_capacity);
...
{
    platform.deallocate_memory(buf_page_begin(buffer));
}
...
allocate_buffers(FixedGapArena *arena, memi at, memi count)
{
    ...
    ch8 *memory = (ch8*)platform.allocate_memory(buf_capacity * count);
